...
class OutettsAppClientFlutter {
  ...
  /// General Library Documentation Undocument By General Corporation & Global Corporation & General Developer
  static ApplicationOutettsDatabase coreDatabaseValue() {
    return coreDatabaseMiniLibrary.valueBuilder(
      builder: (db) {
        return ApplicationOutettsDatabase(db.stateData);
      },
    );
  }
...
extension StateOutettsExtensionFLutter<T extends StatefulWidget> on State<T> {
  ApplicationOutettsDatabase getApplicationOutettsDatabase() {
    return OutettsAppClientFlutter.coreDatabaseValue();
  }

  void saveApplicationOutettsDatabase() async {
    await OutettsAppClientFlutter.saveCoreDatabase();
    return;
  }
}
